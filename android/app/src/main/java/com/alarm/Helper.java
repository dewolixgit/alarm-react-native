...
import com.anonymous.alarmreactnative.R;
...
class Helper {
    ...
    private static PendingIntent createOnDismissedIntent(Context context, String alarmUid, int notificationId) {
        ...
        PendingIntent pendingIntent = null;
        if (Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            pendingIntent = PendingIntent.getBroadcast(context.getApplicationContext(), notificationId, intent, PendingIntent.FLAG_IMMUTABLE);
        }
        else
        {
